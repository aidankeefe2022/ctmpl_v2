...
int testTokenizeCodeSegmentsHard(test_arg) {
    ...
    t_assert(tokens.len == 2);
    t_assert(tokens.data[0].text.length == 4);
    t_assert(tokens.data[1].text.length == 5);
    free(tokens.data);
    arena_free(a);
    test_end
}

int testSelectAndTokenize(test_arg) {
    Arena* a = arena_create(MiB(1));
    t_assert(a)
    String s = STR_LIT("not code %{code}% not code %{more code asdfasfasfasfasf}%");
    DArray_string codeSections = extractTemplCodeSegments(a,&s);
    ...
    Array_CtmplToken tokens = tokenizeCodeSegment(a, *codeSections.data[0]);
    t_assert(tokens.len == 1);
    t_assert(tokens.data[0].text.length == 4);
    free(tokens.data);

    tokens = tokenizeCodeSegment(a, *codeSections.data[1]);
    t_assert(tokens.len == 3);
    t_assert(tokens.data[0].text.length == 4);
    t_assert(tokens.data[1].text.length == 4);
    t_assert(tokens.data[2].text.length == 16);
...
int main() {
    ...
    reg_test(ts, testSelectAndTokenize);
    return run_tests(&ts);
}